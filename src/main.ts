...
function loadDisplaySettings(): DisplaySettings {
  try {
    ...
    const themes: Theme[] = ["midnight", "ocean", "light"];
    return {
...
async function loadUsage(from = currentFrom, to = currentTo) {
  ...
  try {
    currentReport = await invoke<UsageReport>("scan_usage", { from, to, customPrices });
  ...
  return `
    <svg class="trend-svg" viewBox="0 0 ${width} ${height}" preserveAspectRatio="none" aria-label="每日花销趋势图">
      ...
      ${daily.map((item, index) => {
        ...
        return `<circle cx="${x}" cy="${y}" r="3" data-date="${item.date}" data-cost="${fmtMoney(item.totals.costCny)}" />`;
      }).join("")}
    </svg>`;
}
...
function metricGrid(report: UsageReport) {
  ...
  return `
    <section class="metric-grid">
      ...
      <article class="metric-card"><div class="metric-top"><span class="metric-icon input">↘</span><span class="tag subtle">非缓存</span></div><p>缓外输入</p><strong>${fmtTokens(t.inputTokens)}</strong><div class="metric-foot"><span>${fullTokens(t.inputTokens)} tokens</span></div></article>
      <article class="metric-card"><div class="metric-top"><span class="metric-icon cache">⌁</span><span class="tag good">${cachePercent}% 占比</span></div><p>缓内输入</p><strong>${fmtTokens(cacheTotal)}</strong><div class="metric-foot split"><span>读取 ${fmtTokens(t.cacheReadTokens)}</span><span>写入 ${fmtTokens(t.cacheWriteTokens)}</span></div></article>
      <article class="metric-card"><div class="metric-top"><span class="metric-icon output">↗</span><span class="tag subtle">生成</span></div><p>输出</p><strong>${fmtTokens(t.outputTokens)}</strong><div class="metric-foot"><span>${fullTokens(t.outputTokens)} tokens</span></div></article>
    </section>`;
}
...
function modelsPage(report: UsageReport) {
  ...
  return `
    ...
    ${filterBar()}${warningBar(report)}${metricGrid(report)}
    <section class="model-card-grid">${report.models.map((model, index) => `
    ...
    <section class="panel models-panel"><div class="panel-head"><div><p class="panel-kicker">ALL MODELS</p><h2>完整模型明细</h2></div><span class="model-count">${report.models.length} 个模型</span></div>${modelTable(report.models)}</section>`;
}
  ...
  return `
    ...
    <section class="panel settings-section">
      ...
      <div class="theme-grid">
        ${themeCard("midnight", "午夜紫", "默认深色主题", ["#090b11", "#9588ff", "#65d9ae"])}
        ${themeCard("ocean", "深海蓝", "低饱和冷色主题", ["#07131c", "#4ca6d9", "#60d3c1"])}
        ${themeCard("light", "云雾白", "明亮清晰主题", ["#f3f5f8", "#6557c8", "#31866c"])}
      </div>
    </section>
    <section class="panel settings-section">
      ...
      <div class="font-role-grid">
        ...
        ${fontRoleInput("body", "一般正文", "说明文字、标签与普通内容", "sample-body")}
        ${fontRoleInput("secondary", "辅助文字", "注释、副标题、状态与次要信息", "sample-secondary")}
...
function bindEvents() {
  ...
  document.querySelectorAll<SVGCircleElement>(".trend-svg circle").forEach((circle) => {
    circle.addEventListener("mouseenter", () => {
      if (!tooltip) return;
      tooltip.innerHTML = `<strong>${circle.dataset.cost}</strong><span>${circle.dataset.date}</span>`;
